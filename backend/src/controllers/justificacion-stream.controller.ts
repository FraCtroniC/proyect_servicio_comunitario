...
export const JustificacionStreamController = {
  connect: async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      console.log(`[SSE-Justificacion] Conexión establecida para usuario ${decoded.idUsuario}, total: ${clients.size}`);

      req.on('close', () => {
        clients.delete(client);
        console.log(`[SSE-Justificacion] Conexión cerrada para usuario ${decoded.idUsuario}, total: ${clients.size}`);
      });
    } catch (err) {
      console.error('[SSE-Justificacion] Error en connect:', err);
      if (!res.headersSent) {
...
export function broadcastJustificacionEvent(event: { tipo: 'create' | 'delete'; data: any }) {
