...
export const AsistenciaEstudianteStreamController = {
  connect: async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      console.log(`[SSE-AsistenciaEstudiante] Conexión establecida para usuario ${decoded.idUsuario}, total: ${clients.size}`);

      req.on('close', () => {
        clients.delete(client);
        console.log(`[SSE-AsistenciaEstudiante] Conexión cerrada para usuario ${decoded.idUsuario}, total: ${clients.size}`);
      });
    } catch (err) {
      console.error('[SSE-AsistenciaEstudiante] Error en connect:', err);
      if (!res.headersSent) {
...
export function broadcastAsistenciaEstudianteEvent(event: { tipo: 'create' | 'update' | 'batch'; data: any }) {
