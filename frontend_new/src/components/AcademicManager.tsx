...
export default function AcademicManager({ students, currentUserRole, onAddStudent, onUpdateStudentStatus }: AcademicManagerProps) {
  ...
  return (
    <div id="academic-manager-container" className="space-y-6 max-w-6xl mx-auto p-2 md:p-4 selection:bg-indigo-100 selection:text-indigo-900">
      ...
      <div id="academic-header" className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-4">
        <div>
          <h1 id="academic-title" className="text-xl font-bold text-slate-800 flex items-center gap-2">
            ...
            Matrícula y Control de Expedientes
          </h1>
          <p className="text-xs text-slate-500 mt-1">Estructuración técnica escolar de los 5 Años de Educación Media General y Secciones activas.</p>
        </div>
      ...
      {/* Main layout grid */}
      <div id="academic-grid" className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* Left column: Student list explorer with filters (col span 2) */}
        <div id="student-explorer-panel" className="bg-white rounded-xl border border-slate-200/80 p-5 lg:col-span-2 space-y-4 shadow-xs">
          ...
          <div id="explorer-filters-bar" className="flex flex-col md:flex-row md:items-center gap-3 bg-slate-50 p-4 rounded-xl border border-slate-150">
            ...
            <input
              ...
              className="md:ml-auto text-xs p-2 bg-white border border-slate-200 rounded-lg focus:outline-hidden focus:border-indigo-500 min-w-[200px]"
            />
          </div>
        ...
        {/* Right column: Register new Student panel (requires admin rights) */}
        <div id="new-enrollment-panel" className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-xs h-fit space-y-4">
          <div id="new-enrollment-header" className="border-b border-slate-100 pb-3">
          ...
          {!['super_admin', 'control_estudios'].includes(currentUserRole) ? (
            <div id="enrollment-locked" className="p-4 bg-amber-50 rounded-lg border border-amber-200 text-xs text-amber-800 space-y-2">
              <ShieldAlert className="h-5 w-5 text-amber-600" />
              <p className="font-bold">Acceso Administrativo Requerido</p>
              <p className="leading-relaxed">
                Su rol simulado no posee potestades de Control de Estudios (Docentes/Representantes no pueden matricular alumnos). 
                Cambie al perfil de <strong>Director</strong> o <strong>Control de Estudios</strong> en la sección superior "Usuarios" para habilitar esta pestaña transaccional.
              </p>
            </div>
          ) : (
