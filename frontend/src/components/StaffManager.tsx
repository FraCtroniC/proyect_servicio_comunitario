  ...
  return (
    <>
      ...
      <div id="users-grid" className="grid grid-cols-1 gap-6">
        ...
        <div id="users-list-panel" className="bg-white rounded-xl border border-slate-200/80 p-5 space-y-4">
          <div id="users-list-header" className="flex items-center justify-between border-b border-slate-100 pb-3">
            <h3 className="text-base font-bold text-slate-800 tracking-tight">Directorio del Personal & Usuarios</h3>
            <div className="flex items-center gap-4">
              <span className="text-xs bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full font-semibold">Total: {users.length}</span>
            ...
            {visibleUsersCount < users.length && (
              <div className="flex justify-center pt-2 pb-1">
                <button
                  ...
                  className="text-sm font-semibold text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 px-4 py-2 rounded-full transition-colors flex items-center gap-1 cursor-pointer pointer-events-auto"
                >
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Registrar Nuevo Personal">
        {!['super_admin', 'control_estudios'].includes(currentUserRole) ? (
          <div id="add-user-locked" className="p-4 bg-amber-50 rounded-lg border border-amber-200 text-sm text-amber-800 space-y-2">
          ...
          <form id="add-user-form" onSubmit={handleSubmit} className="space-y-4">
            {errorMsg && (
              <div id="user-form-error" className="p-2.5 bg-rose-50 text-rose-800 text-sm rounded-lg border border-rose-200 font-medium">
                {errorMsg}
              </div>
            )}

            <div id="form-name-group" className="space-y-1">
              <label className="text-sm font-bold text-slate-600 uppercase tracking-wide">Nombre Completo</label>
              <input 
                type="text" 
            ...
            <div id="form-cedula-group" className="space-y-1">
              <label className="text-sm font-bold text-slate-600 uppercase tracking-wide">Cédula de Identidad</label>
              <input 
                type="text" 
            ...
            <div id="form-email-group" className="space-y-1">
              <label className="text-sm font-bold text-slate-600 uppercase tracking-wide">Correo Electrónico</label>
              <input 
                type="email" 
            ...
            <div id="form-phone-group" className="space-y-1">
              <label className="text-sm font-bold text-slate-600 uppercase tracking-wide">Teléfono de Contacto (Opcional)</label>
              <input 
                type="text" 
            ...
            <div id="form-role-group" className="space-y-1">
              <label className="text-sm font-bold text-slate-600 uppercase tracking-wide">Rol / Cargo</label>
              <select 
                value={role} 
